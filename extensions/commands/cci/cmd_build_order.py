...
def build_order(conan_api: ConanAPI, parser, *args):
    ...
    recipes = {'wt', 'poco', 'resiprocate', 'gdal', 'oatpp-postgresql', 'apr-util', 'taocpp-taopq', 'quickfix', 'yandex-ozo', 'drogon', 'soci', 'qt', 'cyrus-sasl', 'libpqxx'}
    ...
    exported_list = export_recipes_all_versions(conan_api, recipes, args.repo_path)
...
def export_recipes_all_versions(conan_api, recipes, repo_path):
    results = []
    for recipe in recipes:
        recipe_path = os.path.join(repo_path, "recipes", recipe)
        if not os.path.exists(recipe_path):
            continue
        config_path = os.path.join(recipe_path, "config.yml")
        if not os.path.exists(config_path):
            continue
        with open(config_path) as f2:
            config = yaml.safe_load(f2)
            for version, version_info in config["versions"].items():
                version_folder = os.path.join(recipe_path, version_info["folder"])
                conan_api.command.run(f"export {version_folder} --version={version}")
                results.append(recipe + "/" + version)
    return results
